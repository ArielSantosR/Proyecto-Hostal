        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (!IsPostBack)
                {
                    ...
                    detalle = MiSesionDetalle;

                    XmlSerializer sr4 = new XmlSerializer(typeof(Modelo.DetallePedido));
                    StringWriter writer4 = new StringWriter();
                    sr4.Serialize(writer4, detalle);

                    detalle = service.obtenerDetallePedido(writer4.ToString());

                    MiSesionDetalle = detalle;

                    if (detalle != null)
                    {
                        ddlProducto.SelectedValue = detalle.ID_PRODUCTO + "";
                        txtCantidad.Text = detalle.CANTIDAD + "";
                    }

                }
                ddlProducto.Attributes.Add("disabled", "disabled");
                ...
                if (txtCantidad.Text != string.Empty)
                {
                    if (int.TryParse(txtCantidad.Text, out cantidad))
                    {
                        if (cantidad > 0)
                        {
                            ...
                            {
                                Modelo.DetallePedido detalle = MiSesionDetalle;
                                ...
                                foreach (DetallePedido d in MiSesionD)
                                {
                                    if (detalle.ID_PRODUCTO == d.ID_PRODUCTO &&
                                        detalle.ID_DETALLE_PEDIDO != MiSesionDetalle.ID_DETALLE_PEDIDO)
                                    {
                                        existe = true;
                                    }
                                }
                                ...
                                else
                                {
                                    ...
                                    if (s.EditarDetallePedido(writer3.ToString()))
                                    {
                                        Response.Write("<script language='javascript'>window.alert('Ha modificado el detalle');window.location='../Empleado/WebHistorialPedidos.aspx';</script>");
                                    }
                                    else
                                    {
                                        alerta_exito.Visible = false;
